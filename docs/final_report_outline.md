...
- **Default scenario:** with no goals specified at all, both algorithms fall back to the standard Micromouse competition convention — a 4-cell centre goal area, any one of which ends the run on first arrival — rather than a genuine multi-goal visitation; a capability of the shared framework for ad hoc and GUI runs, not used by the experimental campaign in §4, where every goal-count scenario is placed automatically (§3.5).
...
- Four goal-count scenarios per maze: k=1,2,3,4, each placed automatically by detour-index placement (§3.5).
